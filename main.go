...
=======
	...
	//"os"
>>>>>>> b9480cd5de877c79c63e9d18ca68d6be1ea6d598
	"strings"
	"sync"
	"time"
)
...
<<<<<<< HEAD
	...
	//fmt.Println(ps)
	//fmt.Println(pe)
	//database, err := sql.Open("sqlite3", dirnow )
...
<<<<<<< HEAD
	}else if(ps == 65535 && pe == 65535 && portl != ""){
...
<<<<<<< HEAD
					...
					go func() {
						ChooseMode(fb, conn, port)
						//fmt.Println(pt + content + te + ip)
					}()
...
func LDAPServer(conn net.Conn ,port int )(pt string ,content string , te string,ip string) {
=======
